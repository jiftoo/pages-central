...
class SheetSlider extends HTMLElement {
	...
	connectedCallback() {
		...
		const textArray = this.getAttribute("text")?.split(",") ?? [""];

		const div = document.createElement("div");
		const label = document.createElement("label");
		this.labelNode = document.createElement("span");
		this.labelNode.innerText = this.getAttribute("name") ?? "default name";
		...
		this.input.name = this.getAttribute("name") ?? "default name";
...
class SheetTextInput extends HTMLElement {
	...
	connectedCallback() {
		...
		input.name = this.labelNode.innerText;
...
class SheetSelect extends HTMLElement {
	...
	connectedCallback() {
		...
		const div = document.createElement("div");
		const label = document.createElement("label");
		label.innerText = this.getAttribute("name") ?? "default name";
		label.appendChild(div);
		div.classList.add("bar");
		const select = document.createElement("select");
		select.addEventListener("change", this.handleChange);
		select.name = label.innerText;
		this.input = select;
...
class SheetCollapsible extends HTMLElement {
	...
	connectedCallback() {
		...
		bar1.textContent = this.getAttribute("name") ?? "";
...
async function submitForm(form) {
	...
	document.querySelector("#submit-key > a").href = "?key=" + formKey;
